class CountingElves {
    ...
    private fun splitToStrings() : List<String> {
        val codes = data.split("\n")
        println("Velkost listu ${codes.size}, prvy riadok je ${codes[0]}")
        return codes
    }

    fun sumOffFirstAndLastNumbers() : Int {
        val codes = splitToStrings()
        var sum : Int = 0
        codes.forEach { code ->
            println("Tento riadok je $code")
            val numbersReplaced = replaceSubstringByNumber(code)
            println("Tento riadok je $numbersReplaced")
            val filteredCode = numbersReplaced.filter { it.isDigit() }
            //println("Tento riadok je $filteredCode")
            val number = arrayOf(filteredCode[0], filteredCode[filteredCode.length - 1]).joinToString("")
            println("Cislo je $number")
            sum += number.toInt()
            //println("Celkovy sucet je $sum")
        }
        println("Celkovy sucet je $sum")
        return sum
    }

    fun replaceSubstringByNumber(code: String) : String {
        ...
        var replacedCode : String = code
        numberWords.forEach { (word, number) ->
            replacedCode = replacedCode.replace(word, number)
        }
        return replacedCode
    }
}
